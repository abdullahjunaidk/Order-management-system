...
var (
	...
	SERVICE_NAME = env.GetEnv("SERVICE_NAME", "auth-service")

	// Database Configuration.
	DATABASE_CONNECTION_URI = env.GetEnv("DATABASE_CONNECTION_URI", "mongodb://localhost:27017")
	DATABASE_NAME           = env.GetEnv("DATABASE_NAME", "auth")
	...
	INSTANCE_ID           = env.GetEnv("INSTANCE_ID", "7555fa92-ede1-4986-b492-172e2493193a")
...
func main() {
	...
	authStore := src.NewAuthStore(*mongoAdapter)
	...
	authService := src.NewAuthService(authStore, rabbitMQ)

	authSeed := src.NewAuthSeed(authStore)
	if err := authSeed.SeedSuperAdmin(context.Background()); err != nil {
		log.WithFields(logrus.Fields{"error": err}).Fatal("Failed to Seed Super Admin!")
	}
	...
	go func() {
		if err := src.ListenAndServeGRPC(authService, GRPC_SERVER_PORT); err != nil {
